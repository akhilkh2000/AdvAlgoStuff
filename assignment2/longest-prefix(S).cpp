...
int longestCommonPrefix(string s, int idx, int n) {
	...
	// cout << "TEST\n";
	// cout << (hash[4]*power[21]) % m << endl;
	// cout << (hash[25] - hash[20] + m) % m << endl;
	...
	while (low <= high) {
		...
		if (left) {
			//cout << "mid:" << mid << endl;
			//cout<<(hash[1]-0)*power
			if (((hash[mid] - hash_main)*power[idx]) % m  == (hash[mid + idx] - hash_temp + m) % m) {
				maxLen = mid + 1;
				low = mid + 1;
			} else {
				high = mid - 1;
			...
			if ((hash[mid] - hash_main + m) % m == ((hash[mid - idx] - hash_temp)*power[idx]) % m) {
				//	cout << "mid: " << mid << endl;
				maxLen = mid - idx + 1;
				low = mid + 1;
			} else {
